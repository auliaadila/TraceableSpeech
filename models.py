...
class Generator(torch.nn.Module):
    def __init__(self, h):
        ...
        self.linear_pre = weight_norm(nn.Linear(in_features=(h.Audio["codebook_weight"] + 256), out_features=h.Audio["codebook_weight"]))
    ...
    def fusion(self, x, sign_trait):
        ...
        x = x.transpose(1,2) 
        x = torch.cat((x, sign_trait), dim=2)  # [32, 50, 512] + [32, 50, 256]
        x = self.linear_pre(x) # [32, 50, 768] -> [32, 50, 512]
        x = x.transpose(1,2) 
        
        return x

    def forward(self, x, sign_trait): 
        x = self.fusion(x, sign_trait)  # [32, 512, 50] -> [32, 512, 50]
        x = self.conv_pre(x)
...
class Encoder(torch.nn.Module):
    def __init__(self, h):
        ...
        self.conv_post = Conv1d(512, h.Audio["codebook_weight"], 3, 1, padding=1)
        self.ups.apply(init_weights)
        self.conv_post.apply(init_weights)
    ...
    def embed(self, x, need): 
        ...
        for i in range(min(self.residul_layer, need)):
            ...
            for j in range(self.n_code_groups):
                ...
                embed = self.quantizer_module_residul_list[i][j]
